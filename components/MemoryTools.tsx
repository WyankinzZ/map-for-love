        ...
        <div className="h-fit rounded-[8px] border border-[#D8DDD8]/78 bg-[#FAFBF7]/76 p-5 shadow-[0_12px_28px_rgba(90,102,112,0.06)] backdrop-blur">
          ...
          {config.kind === "favorite" && (
            <div className="relative mt-3" ref={dropdownRef}>
              <div
                className={`flex w-full items-center justify-between rounded-[7px] border ${isCityDropdownOpen ? 'border-[#E8B8C2]' : 'border-[#D8DDD8]'} bg-[#FAFBF7] px-3 py-2 text-sm outline-none transition`}
              >
                <input
                  className="w-full bg-transparent outline-none text-[#5A6670] placeholder-[#5A6670]/50"
                  placeholder="搜索或选择城市"
                  value={citySearch}
                  onChange={(e) => {
                ...
                />
                <ChevronDown 
                  className={`h-4 w-4 shrink-0 text-[#5A6670]/50 transition-transform cursor-pointer ${isCityDropdownOpen ? 'rotate-180' : ''}`} 
                  onClick={() => {
              ...
              {isCityDropdownOpen && (
                <div className="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-[7px] border border-[#D8DDD8] bg-[#FAFBF7] py-1 shadow-lg">
                  {cityOptions.filter(c => c.name.includes(citySearch)).map((city) => (
                    <div
                      key={city.id}
                      className={`cursor-pointer px-3 py-2 text-sm transition hover:bg-[#F5DCE0]/50 ${city.id === cityId ? 'bg-[#F5DCE0]/80 font-semibold text-[#E8B8C2]' : 'text-[#5A6670]'}`}
                      onClick={() => {
                        setCityId(city.id);
                  ...
                  {cityOptions.filter(c => c.name.includes(citySearch)).length === 0 && (
                    <div className="px-3 py-4 text-center text-sm text-[#5A6670]/50">
                      未找到相关城市
                    </div>
                  )}
                </div>
